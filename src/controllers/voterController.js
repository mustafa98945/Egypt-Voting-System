...
const processBase64AndUpload = async (base64String, fileName, folder = 'voters') => {
    try {
        ...
        try {
            // تحسين جودة الصورة وتقليل حجمها
            ...
            console.error("Sharp processing failed, uploading raw buffer:", sharpError.message);
...
// --- 2. التحقق المبدئي والـ Auto-fill (الشاشة الأولى) ---
exports.verifyBeforeRegister = async (req, res) => {
    try {
        ...
        // أ- التحقق من وجود الشخص في السجل المدني
        ...
        // ب- التأكد أن الشخص لم يسجل حساباً من قبل
        const duplicate = await Voter.checkDuplicate(email, national_id);
        if (duplicate) {
            const msg = duplicate.email_exists ? "البريد الإلكتروني مسجل بالفعل" : "الرقم القومي مسجل بالفعل";
            return res.status(400).json({ success: false, message: msg });
        }

        // إرسال البيانات للفرونت إند لملء الشاشات التالية تلقائياً
        res.json({ 
            success: true,
...
// --- 3. إتمام عملية التسجيل (تشمل منطق الحزب السياسي) ---
exports.registerVoter = async (req, res) => {
    try {
        const data = req.body;

        // إعادة التحقق السريع لضمان الأمان
        const citizen = await Voter.verifyInRegistry(data.national_id, data.birth_date, data.expiry_date);
        if (!citizen) return res.status(401).json({ success: false, message: "فشل التحقق من البيانات" });

        // --- منطق الحزب السياسي (شاشة Figma) ---
    ...
    data.party_card_url, fileName, 'voters_cards/voters/party_cards'
        ...
        const newVoter = await Voter.create({
            ...
            party_card_url: partyCardUrl // سيبقى null إذا داس المستخدم No
        });
...
// --- 4. تسجيل الدخول (مع وضع الدائرة في التوكن) ---
exports.login = async (req, res) => {
    try {
        ...
        if (isFaceAuthenticated && national_id) {
            ...
            if (!voter) return res.status(404).json({ success: false, message: "الحساب غير موجود" });
        } 
        ...
        // إنشاء التوكن وإضافة الدائرة الانتخابية (مهم جداً للفلترة) 🎯
...
// --- 5. جلب بيانات الكارت الشخصي ---
exports.getVoterCard = async (req, res) => {
    try {
        // req.user.id يأتي من الـ authMiddleware
